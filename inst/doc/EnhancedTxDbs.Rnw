...
if(interactive()) {
  ...
  EnhancedTxDb <- 
    createTxDbFromEnsembl(67, annotInfo=TRUE,
                          ensemblApi=eAPI, 
                          species="human", user="anonuser", 
                          host="madb.i-med.ac.at", 
                          pass="", chr="19")
} else {
  path <- system.file("extdata", package="EnhancedTxDbs")
  fn <- paste(path, "ens67chr19", sep="/")
...
version 67. We can easily subset the transcripts to those biotypes
